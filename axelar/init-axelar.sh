...
$BINARY init "$MONIKER" --chain-id ${CHAIN_ID} --home ${HOME} > /dev/null 2>&1 && echo "Initialized new blockchain with chain ID ${CHAIN_ID}"

# edit the app.toml file to enable the API and swagger
gsed -i '/\[api\]/,/\[/ s/enable = false/enable = true/' "$HOME"/config/app.toml
gsed -i '/\[api\]/,/\[/ s/swagger = false/swagger = true/' "$HOME"/config/app.toml

# staking/governance token is hardcoded in config, change this
gsed -i "s/\"stake\"/\"$DENOM\"/" "$HOME"/config/genesis.json && echo "Updated staking token to $DENOM"
...
$BINARY set-genesis-mint --inflation-min 0 --inflation-max 0 --inflation-max-rate-change 0 --home ${HOME}
$BINARY set-genesis-gov --minimum-deposit "100000000${DENOM}" --max-deposit-period 90s --voting-period 90s --home ${HOME}
$BINARY set-genesis-reward --external-chain-voting-inflation-rate 0 --home ${HOME}
$BINARY set-genesis-slashing --signed-blocks-window 35000 --min-signed-per-window 0.50 --home ${HOME} \
--downtime-jail-duration 600s --slash-fraction-double-sign 0.02 --slash-fraction-downtime 0.0001 --home ${HOME}
$BINARY set-genesis-snapshot --min-proxy-balance 5000000 --home ${HOME}
$BINARY set-genesis-staking  --unbonding-period 168h --max-validators 50 --bond-denom "$DENOM" --home ${HOME}
$BINARY set-genesis-chain-params evm Ethereum --evm-network-name ethereum --evm-chain-id 5 --network ethereum --confirmation-height 1 --revote-locking-period 5 --home ${HOME}

GOV_1_KEY="$($BINARY keys show gov1 ${DEFAULT_KEYS_FLAGS} -p)"
GOV_2_KEY="$($BINARY keys show gov2 ${DEFAULT_KEYS_FLAGS} -p)"
$BINARY set-governance-key 1 "$GOV_1_KEY" "$GOV_2_KEY" --home ${HOME}
