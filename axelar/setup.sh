...
EVM_DENOM=uCudos
EVM_TOKEN_NAME="ethereum uCudos"
EVM_TOKEN_SYMBOL=CudosToken
DIR="$(dirname "$0")"
...
TOKEN_ADDRESS=$(jq -r '.[0].CudosToken.tokenDeployAddress' testnet/evm-testnet/chain-config/local-evm.json)
TX_HASH=$(jq -r '.[0].CudosToken.tokenDeployTxHash' testnet/evm-testnet/chain-config/local-evm.json)
